      ...
      <section className="rounded-xl border border-zinc-200 bg-white p-5">
        ...
        <form className="mt-4 grid gap-3" onSubmit={createProduct}>
          ...
          <div className="grid grid-cols-3 gap-2">
            <input type="number" step="0.01" value={form.price} onChange={(e) => setForm({ ...form, price: Number(e.target.value) })} placeholder="Price" className="rounded border border-zinc-300 px-3 py-2" />
          ...
          {products.map((product) => (
            <article key={product.id} className="rounded border border-zinc-200 p-3">
              ...
              <div className="mt-2 grid grid-cols-[1fr_1fr_auto] items-center gap-2">
                <input type="number" step="0.01" value={product.price} className="rounded border border-zinc-300 px-2 py-1" onChange={(e) => updateProduct(product, { price: Number(e.target.value) })} />
